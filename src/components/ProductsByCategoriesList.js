...
const ProductList = ({ data, setCart, cart }) => {
    ...
    return (
        <div>
            ...
            {(groupedProducts && typeof groupedProducts === 'object') && Object.entries(groupedProducts).map(([category, products]) => (
                <div key={category}>
                            ...
                            <div key={product.id} className="flex items-center justify-between">
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        type="checkbox" /> <p>{product.name}</p>
                                </div>
                                <div>
                                    <NumericFormat
                                        value={product.price}
                                        ...
                                        renderText={value => <h3 className="text-[14px] md:text-[16px] lg:text-[18px] text-[#CA5834] font-bold">
                                            {value}
                                        </h3>}
                                    />
